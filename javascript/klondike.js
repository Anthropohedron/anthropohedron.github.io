...
function klondike(x, y) {
  ...
  this.discardLoc = this.newLocation(this.x, this.y, 0, 0);
...
reposition : function() {
               var card = this.stockLoc.placeholder;
               card.rect.updateFromNode(card.node);
               var xdist = card.rect.width() + 5;
               var ydist = card.rect.height() + 5;
               var yoffset = Math.floor(ydist / 3);
               var xoffset = Math.floor(xdist / 3);
               this.stockLoc.moveTo(this.x, this.y);
               this.discardLoc.moveTo(xdist+this.x, this.y);
               this.discardLoc.setOffset(xoffset, 0);
               this.aces['spades'].moveTo((xdist*3)+this.x, this.y);
               this.aces['hearts'].moveTo((xdist*4)+this.x, this.y);
               this.aces['diamonds'].moveTo((xdist*5)+this.x, this.y);
               this.aces['clubs'].moveTo((xdist*6)+this.x, this.y);
               for (i=0;i<7;++i) {
